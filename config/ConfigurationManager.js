 ...
 * 
 * This module provides centralized configuration management for QueueCraft application.
...
const debug   = require('debug');     // Debug logging utility
const _       = require('lodash');    // Utility library
const convict = require('convict');   // Configuration validation library
...
function getApplicationRoot(){
    ...
    // Start from the main module filename, or current working directory as fallback
    if (require.main && require.main.filename) {
        path0 = path.dirname(require.main.filename);
    } else {
        path0 = process.cwd();
    }
    ...
    _
        ...
        .each(function(file){
            ...
            if(stats.isDirectory()){
                ...
                const propName = path.basename(file, ".json");
...
ConfigurationManager.prototype.override = function(){
    const appRoot  = this.config.get('app.root');
...
ConfigurationManager.prototype.load = function(){
    ...
    this.schema.app = this.schema.app || {};
...
if(!module.parent){
    ...
    if(action === "dump"){
        configMgr.load();
        const properties  = configMgr.config.getProperties();
